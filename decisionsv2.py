...
def draw_tree(screen, node_x, node_y, angle, depth):
    ...
    # Randomly choose how many branches to create at this point (between 2 and 4)
...
def visualize_decision_tree():
    ...
    while running:
        ...
        if not tree_drawn:
            ...
            # Calculate horizontal spacing between trees
            spacing = SCREEN_WIDTH // (NUM_TREES + 1)

            # Start drawing multiple trees from left to right
            for i in range(NUM_TREES):
                start_x = (i + 1) * spacing  # Evenly distribute the starting positions
                start_y = SCREEN_HEIGHT // 2  # Vertically centered
                initial_angle = 0  # Angle 0 for rightward movement
